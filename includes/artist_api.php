...
define('AVATAR_UPLOAD_DIR', '../uploads/artist_avatars/');
// Xử lý file ảnh đầu vào, trả về url của ảnh hoặc thất bại.
function handleAvatarUpload($file_input) {
    ...
    if (move_uploaded_file($file['tmp_name'], $targetPath)) {
        return 'uploads/artist_avatar/' . $newName;
    }

    throw new Exception('Upload avatar thất bại.');
}
// Lấy hành động để thực hiện chức năng
$action = $_GET['action'] ?? $_POST['action'] ?? '';
// Đọc CSDL để lấy các thông tin cần hiện
if ($action === 'read') {
    $search = $_POST['search'] ?? '';
    try {
        if ($search !== '') {
...
// Tạo nghệ sĩ mới và thêm vào CSDL
if ($action === 'create') {
...
// Lấy thông tin của nghệ sĩ và cập nhật thông tin
if ($action === 'update') {
    $id = $_POST['artist_id'] ?? '';
    $name = trim($_POST['artist_name'] ?? '');
    $biography = trim($_POST['biography'] ?? '');
