...
// Lấy danh sách nghệ sĩ để hiển thị ở Dropdown
$artists = [];
$artistQuery = $conn->query("SELECT Artist_id, Artist_name FROM Artists ORDER BY Artist_name ASC");
if ($artistQuery && $artistQuery->num_rows > 0) {
    while ($row = $artistQuery->fetch_assoc()) {
        $artists[] = $row;
    }
}

// Lấy danh sách Album để hiển thị ở Dropdown phần thêm bài hát
$albums = [];
$albumQuery = $conn->query("SELECT Album_id, Album_title FROM Albums ORDER BY Album_title ASC");
...
<body>
    ...
    <main class="main-content">
        ...
        <div id="song-crud-modal" class="admin-modal-overlay" style="display: none;">
            <div class="admin-modal-content" style="width: 600px; height:650px; overflow-y: auto;">
                ...
                <form id="song-crud-form" enctype="multipart/form-data" onsubmit="submitSong(event)">
                    ...
                    <div class="form-group-modern">
                        <label>Song Avatar (JPG/PNG)</label>
                        <input type="file" id="song_avatar" name="song_avatar" accept="image/png, image/jpeg, image/jpg">
                        <small id="current-song-avatar-name" style="color: #666; display: block; margin-top: 5px;"></small>
                    </div>
                    <div class="form-group-modern">
                        <label>Avatar URL (fallback)</label>
                        <input type="text" id="song_image_url" name="song_image_url" placeholder="./img/default-song.jpg">
                    </div>
                    <div class="form-group-modern">
                        <label>Lyrics</label>
                    ...
                    <div class="form-group-modern">
                        ...
                        <select id="album_id" name="album_id">
                            ...
                            <?php foreach($albums as $al): ?>
                                <option value="<?php echo $al['Album_id']; ?>">
                                    <?php echo htmlspecialchars($al['Album_title']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group-modern">
                        ...
                        <select id="artist_id" name="artist_id" required>
                            ...
                            <?php foreach($artists as $ar): ?>
                                <option value="<?php echo $ar['Artist_id']; ?>">
                                    <?php echo htmlspecialchars($ar['Artist_name']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
