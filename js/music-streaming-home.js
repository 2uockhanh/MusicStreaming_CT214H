...
function playSong(url, title, artist, img) {
    ...
    if (audioUrl && !audioUrl.startsWith('http')) {
        ...
        if (!audioUrl.startsWith('MusicStreaming/')) {
            audioUrl = '/MusicStreaming/' + audioUrl; 
        } else {
            audioUrl = '/' + audioUrl;
        }
    }
    ...
    if (volumeBar) {
        volumeBar.addEventListener('input', () => {
            ...
            if (volumeBar.value == 0) {
                document.getElementById('volume-img').src = './img/off-volume.png';
            } else {
                document.getElementById('volume-img').src = './img/volume.png';
            }
        });
    }
